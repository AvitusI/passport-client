...
const Comment = ({ comment }) => {
  ...
  const sendLike = async (sentData) => {
    if (likeStatus) {
      const { data } = await axios.post("https://shownext1-7sh63dv9.b4a.run/api/comments/unlike", sentData, { withCredentials: true })
      return data
    }
    else {
      const { data } = await axios.post("https://shownext1-7sh63dv9.b4a.run/api/comments/like", sentData, { withCredentials: true })
      return data
    }
  }

  const deleteComment = async () => {
    const { data } = await axios.delete(`https://shownext1-7sh63dv9.b4a.run/api/comments/${comment._id}`, { withCredentials: true })
    return data
  }
