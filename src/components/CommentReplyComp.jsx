...
const fetchReplies = async ({ queryKey }) => {
    ...
    const { data } = await axios.get(`https://shownext1-7sh63dv9.b4a.run/api/comments/replies/${commentId}`, { withCredentials: true })
    return data
}

const replyComment = async (sentData) => {
    const { data } = await axios.post("https://shownext1-7sh63dv9.b4a.run/api/comments/reply", sentData, { withCredentials: true })
    return data
}
